...
class CIFAR10DatasetHparams(DatasetHparams, SyntheticHparamsMixin):
    ...
    Args:
        ...
        ffcv_dir (str): A directory containing train/val <file>.beton files. If these files don't exist and
            ``ffcv_write_dataset`` is ``True``, train/val <file>.beton files will be created in this dir. Default: ``"/tmp"``.
        ffcv_dest_train (str): <file>.beton file that has training samples. Default: ``"cifar_train.beton"``.
        ffcv_dest_val (str): <file>.beton file that has validation samples. Default: ``"cifar_val.beton"``.
        ffcv_write_dataset (std): Whether to create dataset in FFCV format (<file>.beton) if it doesn't exist. Default:
        ``False``.
    """
    ...
    ffcv_dir: str = hp.optional(
        "A directory containing train/val <file>.beton files. If these files don't exist and ffcv_write_dataset is true, train/val <file>.beton files will be created in this dir.",
        default="/tmp")
    ffcv_dest_train: str = hp.optional("<file>.beton file that has training samples", default="cifar_train.beton")
    ffcv_dest_val: str = hp.optional("<file>.beton file that has validation samples", default="cifar_val.beton")
    ffcv_write_dataset: bool = hp.optional(
        "Whether to create dataset in FFCV format (<file>.beton) if it doesn't exist", default=False)
    ...
    def initialize_object(self, batch_size: int, dataloader_hparams: DataLoaderHparams) -> DataLoader:
        ...
        if self.use_synthetic:
            ...
            if self.ffcv_write_dataset and not os.path.exists(dataset_filepath):
                log.info(f"Writing dataset in FFCV <file>.beton format to {dataset_filepath}.")
                ...
                writer = ffcv.writer.DatasetWriter(dataset_filepath, {
